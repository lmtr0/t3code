...
export function parseClaudeInitializationSkills(
  input: unknown,
  ...
  const skillsByIdentity = new Map<string, ServerProviderSkill>();
  for (const item of input) {
    ...
    skillsByIdentity.set(JSON.stringify([name.toLowerCase(), path, scope?.toLowerCase() ?? ""]), {
      name,
      path,
      enabled,
      ...(description ? { description } : {}),
